...
namespace ELearn.Common.Utilities
{
    ...
    {
        public static Expression<Func<T, bool>> CombiningExpressions<T>(Expression<Func<T, bool>> func1, Expression<Func<T, bool>> func2, ExpressionsType expressionsType)
        {
            if (expressionsType == ExpressionsType.And)
            {
                return func1.CombineWithAndAlso(func2);
            }
            else if (expressionsType == ExpressionsType.Or)
            {
